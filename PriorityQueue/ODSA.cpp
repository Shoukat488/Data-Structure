...
class ODSA{
	...
	public:
	     ...
	     ODSA(); // it is default constructor which assigns 0 to size and data
	     ODSA(int Size); //Construtor to intialize and to allocate memory for variable memebers;
		 ODSA(const ODSA& rhs); // copy constructor
	     ~ODSA();
	     int Size(); // it returns size of array
	     void ReSize(int nSize);
		 ODSA& operator = (const ODSA& rhs); // assignment operator overloading
		 ...
		 friend ifstream& operator >> ( ifstream& infile , ODSA& rhs);
		 friend ofstream& operator << ( ofstream& outfile , ODSA& rhs);
};
...
int ODSA::get(int index)
{
	return Data[index];
}
void ODSA::set(int element ,int index)
{
	Data[index] = element;
}
// assignment operator overloading function
ODSA& ODSA:: operator=( const ODSA & rhs)
{
    if (this  != &rhs)
        {
        int s=rhs.size; 
...
const int& ODSA::operator [] (int i) const {   
...
ifstream& operator >> (ifstream& infile, ODSA & rhs)
{
	// int n = rhs.Size();
	// while(!infile.eof())
...
ofstream& operator << (ofstream& outfile, ODSA & rhs)
{
	// int n = rhs.Size();
	// int count = 0 ;
